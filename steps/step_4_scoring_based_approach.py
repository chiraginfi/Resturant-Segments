...
df = pd.read_csv("/mnt/data/image_recognition/brown_forman_req/output/restaurant_brand_data_include_all_attributes_mentions.csv")
...
def classify_restaurant(row):
    ...
    cost = row.get("cost_for_two_numeric")
    ...
    if price_score is None:
        ...
        if price_score is not None:
            price_score = max(0, price_score - 2)   # larger haircut: estimated cost
...
df.to_csv(
    "/mnt/data/image_recognition/brown_forman_req/output/restaurant_brand_data_include_all_attributes_mentions_segmented.csv",
    index=False
)
print("Done. Shape:", df.shape)
print(df[["restaurant_name", "segment_score", "restaurant_segment", "score_breakdown"]].head(10).to_string(index=False))
